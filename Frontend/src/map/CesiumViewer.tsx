...
const getAlgorithmColor = ( algorithm?: string ): Cesium.Color => {
    switch ( algorithm ) {
        ...
        case 'rl':
            return Cesium.Color.LIME;           // XANH LÁ NEON - RL intelligent routing (avoids traps)
        case 'astar':
            return Cesium.Color.GOLD;           // VÀNG - A* routing
        default:
...
const CesiumViewer: React.FC<CesiumViewerProps> = ( { nodes, routingPath, activePackets = [], onClearPaths, onPathClick, onTerminalCreated } ) => {
    ...
    const drawRoutingPath = useCallback( ( path: RoutingPath, pathId: string, isPacketPath: boolean = false ) => {
        ...
        // Store segment info with altitude and type for proper handling
        interface SegmentInfo {
            pos: Cesium.Cartesian3;
            altitude: number;
            isGroundBased: boolean; // terminal, ground_station = true, satellite = false
        }
        const segmentInfos: SegmentInfo[] = [];

        // First, collect all original positions with segment info
        path.path.forEach( ( segment, index ) => {
            const altitude = segment.position.altitude || 0;
            ...
            // Check if this is a ground-based node (terminal or ground station)
            const nodeType = segment.name?.toLowerCase() || '';
            ...
            const isGroundBased =
                ...
                nodeType.includes( 'gs-' ) ||
                altitude < 1000; // Less than 1km altitude = ground-based

            segmentInfos.push( { pos, altitude, isGroundBased } );

            // Debug: Log each segment position
            console.log( `  📍 Segment ${ index }: ${ segment.type } "${ segment.name }" altitude=${ altitude }m ground=${ isGroundBased }` );
        } );

        // Create smooth curved path - preserve satellite altitudes!
        for ( let i = 0; i < segmentInfos.length - 1; i++ ) {
            const startInfo = segmentInfos[ i ];
            const endInfo = segmentInfos[ i + 1 ];
            const isFirstSegment = ( i === 0 );
            const isLastSegment = ( i === segmentInfos.length - 2 );

            // Get cartographic positions for proper elevation
            const startCarto = Cesium.Cartographic.fromCartesian( startInfo.pos );
            const endCarto = Cesium.Cartographic.fromCartesian( endInfo.pos );

            // Calculate arc height based on distance
            const distance = Cesium.Cartesian3.distance( startInfo.pos, endInfo.pos );
            // Smaller arc for satellite-to-satellite links (they're already high)
            const arcMultiplier = ( !startInfo.isGroundBased && !endInfo.isGroundBased ) ? 0.1 : 0.2;
            const arcHeight = Math.max( 50000, distance * arcMultiplier );

            // Interpolate positions along the arc
            const segments = Math.max( 20, Math.floor( distance / 80000 ) );

            for ( let j = 0; j <= segments; j++ ) {
                const t = j / segments;

                // Linear interpolation of lat/lon
                const lat = startCarto.latitude + t * ( endCarto.latitude - startCarto.latitude );
                const lon = startCarto.longitude + t * ( endCarto.longitude - startCarto.longitude );

                // Sinusoidal arc factor
                const arcFactor = Math.sin( Math.PI * t );

                // Calculate elevation based on segment types
                let elevation: number;

                // Start altitude
                const startAlt = startInfo.isGroundBased ? 100 : startInfo.altitude;
                // End altitude
                const endAlt = endInfo.isGroundBased ? 100 : endInfo.altitude;
                // Interpolated base altitude
                const baseAlt = startAlt + t * ( endAlt - startAlt );

                if ( isFirstSegment && j === 0 && startInfo.isGroundBased ) {
                    elevation = 100; // Start clamped to ground for terminals
                } else if ( isLastSegment && j === segments && endInfo.isGroundBased ) {
                    elevation = 100; // End clamped to ground for terminals
                } else {
                    // Arc above base altitude
                    elevation = baseAlt + arcHeight * arcFactor;
                }

                const position = Cesium.Cartesian3.fromRadians( lon, lat, elevation );
                positions.push( position );
            }
        }

        // Don't add final position again - it's already added in the loop
